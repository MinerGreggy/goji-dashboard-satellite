...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'goji-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('goji').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('goji')[service]);
